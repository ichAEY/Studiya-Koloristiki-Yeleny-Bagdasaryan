...
reviews.innerHTML=`<div class="tn30-reviews"><p class="tn22-kicker">Отзывы</p><h2>Что говорят о нас</h2><div class="tn30-score"><strong>5.0</strong><div class="tn30-stars" aria-label="5 звёзд">★★★★★</div><div class="tn30-count">165 отзывов · Яндекс Карты</div></div><div class="tn30-review-stage">${reviewLanes.map((lane,i)=>`<div class="tn30-lane" data-review-lane="${i}"><div class="tn30-track">${Array.from({length:5},(_,setIndex)=>`<div class="tn30-review-set" aria-hidden="${setIndex===2?'false':'true'}">${lane.map(reviewCard).join('')}</div>`).join('')}</div></div>`).join('')}</div><a class="tn30-review-all" href="${YANDEX_REVIEWS}" target="_blank" rel="noopener">Смотреть все отзывы →</a></div>`;
const reviewStage=reviews.querySelector('.tn30-review-stage');
const reviewTracks=[...reviews.querySelectorAll('.tn30-track')];
reviews.querySelectorAll('.tn30-review-card').forEach(card=>card.setAttribute('draggable','false'));
const reviewWidths=reviewTracks.map(()=>0),reviewOffsets=reviewTracks.map(()=>0);
let reviewFrame=0,reviewLastFrame=0,reviewPaused=false,reviewPointerStart=null,reviewStartOffsets=[],reviewWasDragged=false,reviewSuppressClick=false,reviewResumeTimer=0;
function normalizeReviewOffset(offset,width){if(!width)return offset;while(offset<=-width*3)offset+=width*2;while(offset>-width)offset-=width*2;return offset}
function setReviewOffset(index,next){const width=reviewWidths[index];const normalized=normalizeReviewOffset(next,width);reviewOffsets[index]=normalized;reviewTracks[index].style.transform=`translate3d(${normalized}px,0,0)`}
function measureReviews(){reviewTracks.forEach((track,index)=>{const set=track.querySelector('.tn30-review-set');const nextWidth=set?set.getBoundingClientRect().width:0;if(!nextWidth)return;const prev=reviewWidths[index];reviewWidths[index]=nextWidth;setReviewOffset(index,prev?(reviewOffsets[index]/prev)*nextWidth:-nextWidth*2)})}
function moveReviewsFrame(time){if(!reviewLastFrame)reviewLastFrame=time;const elapsed=Math.min(time-reviewLastFrame,34);reviewLastFrame=time;if(!reviewPaused&&document.visibilityState==='visible'){reviewTracks.forEach((_,index)=>setReviewOffset(index,reviewOffsets[index]-elapsed*.032))}reviewFrame=requestAnimationFrame(moveReviewsFrame)}
function pauseReviews(clientX){clearTimeout(reviewResumeTimer);reviewPaused=true;reviewStage.classList.add('is-paused');reviewPointerStart=clientX??null;reviewStartOffsets=reviewOffsets.slice();reviewWasDragged=false}
function moveReviewsByPointer(clientX){if(reviewPointerStart===null)return;const distance=clientX-reviewPointerStart;if(Math.abs(distance)>7)reviewWasDragged=true;reviewTracks.forEach((_,index)=>setReviewOffset(index,reviewStartOffsets[index]+distance))}
function resumeReviews(delay=0){clearTimeout(reviewResumeTimer);const done=()=>{reviewPointerStart=null;reviewPaused=false;reviewStage.classList.remove('is-paused')};if(delay)reviewResumeTimer=setTimeout(done,delay);else done()}
reviewStage.addEventListener('pointerdown',e=>{if(!e.isPrimary)return;pauseReviews(e.clientX)});
reviewStage.addEventListener('pointermove',e=>{if(!e.isPrimary||!reviewPaused||reviewPointerStart===null)return;if(Math.abs(e.clientX-reviewPointerStart)>7&&!reviewStage.hasPointerCapture(e.pointerId)){try{reviewStage.setPointerCapture(e.pointerId)}catch(_){}}moveReviewsByPointer(e.clientX)});
reviewStage.addEventListener('pointerup',e=>{if(reviewStage.hasPointerCapture(e.pointerId)){try{reviewStage.releasePointerCapture(e.pointerId)}catch(_){}}reviewSuppressClick=reviewWasDragged;resumeReviews(180)});
reviewStage.addEventListener('pointercancel',()=>resumeReviews(180));
reviewStage.addEventListener('click',e=>{if(!reviewSuppressClick)return;e.preventDefault();e.stopPropagation();reviewSuppressClick=false},true);
reviewStage.addEventListener('wheel',e=>{const horizontal=e.shiftKey?e.deltaY:e.deltaX;if(Math.abs(horizontal)<1||Math.abs(horizontal)<Math.abs(e.deltaY)*.55)return;e.preventDefault();pauseReviews();reviewTracks.forEach((_,index)=>setReviewOffset(index,reviewOffsets[index]-horizontal*1.12));resumeReviews(520)},{passive:false});
if('ResizeObserver' in window){const reviewResizeObserver=new ResizeObserver(measureReviews);reviewResizeObserver.observe(reviewStage)}else window.addEventListener('resize',measureReviews,{passive:true});
requestAnimationFrame(()=>{measureReviews();reviewFrame=requestAnimationFrame(moveReviewsFrame)});
