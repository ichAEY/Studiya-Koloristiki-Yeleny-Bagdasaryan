...
function fitHeroTitle(){
  if(!heroTitleEl||!heroCardEl)return;
  const target=Math.max(190,heroCardEl.clientWidth-88);
  let size=Math.min(18,Math.max(12.5,window.innerWidth*.046));
  heroTitleEl.style.width=target+'px';
  heroTitleEl.style.maxWidth=target+'px';
  heroTitleEl.style.fontSize=size+'px';
  while(heroTitleEl.scrollWidth>target&&size>12.5){
    size=Math.max(12.5,size-.25);
    heroTitleEl.style.fontSize=size+'px';
  }
}
...
new MutationObserver(fitHeroTitle).observe(heroTitleEl,{childList:true,characterData:true,subtree:true});
